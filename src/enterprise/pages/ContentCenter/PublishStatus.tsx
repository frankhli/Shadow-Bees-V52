...
export default function PublishStatus() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (contentsResponse.success) {
          // 转换 API 数据为页面需要的格式
          const transformedData: ContentItem[] = contentsResponse.data.list.map(item => ({
            id: item.id,
            platform: (item.platforms?.[0] || 'wechat') as Platform,
            title: item.title,
            content: item.content,
            status: item.status === 'published' ? 'published' : 'draft',
            images: item.images || [],
            createdAt: item.createdAt,
            price: item.metrics ? Math.floor(item.metrics.views / 100) : 520,
            publishedAt: item.publishedAt,
            performance: item.metrics ? {
              impressions: item.metrics.views * 10,
              clicks: item.metrics.likes * 5,
              inquiries: item.metrics.comments * 2,
        ...
        const txnResponse = await contentApi.getContentTransactions({
        ...
        if (txnResponse.success) {
          setTransactions(txnResponse.data.list);
        }
      } catch (error) {
  ...
  const newContentsStats = useMemo(() => {
    const stats: Record<string, { published: number; running: number; deals: number; gmv: number }> = {
      ...
      wechat: { published: 0, running: 0, deals: 0, gmv: 0 },
      douyin: { published: 0, running: 0, deals: 0, gmv: 0 },
    };
  ...
  const activeContentsStats = useMemo(() => {
    const stats: Record<string, { active: number; deals: number; gmv: number; totalContents: number }> = {
      ...
      wechat: { active: 0, deals: 0, gmv: 0, totalContents: 0 },
      douyin: { active: 0, deals: 0, gmv: 0, totalContents: 0 },
    };
  ...
  const retentionStats = useMemo(() => {
    ...
    const stats: Record<string, { 
      ...
      wechat: { prevPeriodContents: 0, retainedContents: 0, retentionRate: 0, retainedGMV: 0 },
      douyin: { prevPeriodContents: 0, retainedContents: 0, retentionRate: 0, retainedGMV: 0 },
    };
  ...
  const funnelData = useMemo(() => {
    ...
    // 再根据平台筛选
    if (selectedPlatform) {
      sourceContents = sourceContents.filter(c => c.platform === selectedPlatform);
      sourceTransactions = sourceTransactions.filter(t => t.platform === selectedPlatform);
    }
    ...
    const totalConversions = sourceTransactions.length; // 用筛选后的实际成交数
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-gray-900">发布状态</h1>
              ...
              >
                <div className="relative">
                  <input
                    ...
                    className="px-2 py-1 pr-8 bg-white border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:border-neon-cyan/50 [&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:opacity-50 [&::-webkit-calendar-picker-indicator]:cursor-pointer"
                  />
          ...
          ? Object.entries(newContentsStats).map(([platform, stats]) => {
              ...
              return (
                <div
                  ...
                  className={`bg-gray-100 rounded-xl border p-6 cursor-pointer transition-all ${
                    selectedPlatform === platform ? 'border-opacity-100' : 'border-gray-200 hover:border-opacity-50'
                  }`}
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">新增内容成交 {stats.deals}单</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center text-sm">
                    <div className="bg-bg-tertiary rounded p-2">
                      <div className="text-neon-cyan font-mono">{stats.published}</div>
                      <div className="text-xs text-gray-500">新增发布</div>
                    </div>
                    <div className="bg-bg-tertiary rounded p-2">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center text-sm">
                      <div className={`rounded p-2 border ${timeRange === 'today' ? 'bg-neon-green/10 border-neon-green/30' : 'bg-neon-amber/10 border-neon-amber/30'}`}>
                        ...
                        <div className="text-xs text-gray-500">{timeRange === 'today' ? '活跃中' : '有成交'}</div>
                      </div>
                      <div className="bg-bg-tertiary rounded p-2">
                        <div className="text-neon-amber font-mono">{stats.deals}</div>
                        <div className="text-xs text-gray-500">{timeRangeOptions.find(o => o.key === timeRange)?.label || '今日'}成交</div>
                      </div>
                      <div className="bg-bg-tertiary rounded p-2">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-500">{timeRangeOptions.find(o => o.key === timeRange)?.label || '今日'}GMV</span>
                        <span className="text-xl font-mono" style={{ color: info.color }}>
                          {formatPrice(stats.gmv)}
                        </span>
                      </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <div className="font-semibold" style={{ color: info.color }}>{info.name}</div>
                        <div className="text-xs text-gray-500">上周期发布的内容</div>
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-center text-sm">
                      <div className="bg-bg-tertiary rounded p-2">
                        <div className="text-neon-cyan font-mono">{stats.prevPeriodContents}</div>
                        <div className="text-xs text-gray-500">上周期发布</div>
                      </div>
                      <div className="bg-neon-purple/10 rounded p-2 border border-neon-purple/30">
                        ...
                        <div className="text-xs text-gray-500">本周期仍成交</div>
                      </div>
                      <div className="bg-bg-tertiary rounded p-2">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-500">留存内容贡献GMV</span>
                        <span className="text-xl font-mono" style={{ color: info.color }}>
      ...
      <div className="grid grid-cols-12 gap-6">
        {/* 内容明细列表 */}
        <div className="col-span-8 bg-gray-100 rounded-xl border border-gray-200 p-6">
          ...
          {platformFilteredContents.length === 0 ? (
            ...
            <div className="space-y-4">
              {platformFilteredContents.map((content) => {
                ...
                // 实时统计该内容在时间范围内的成交数和GMV
                const contentDeals = filteredTransactions.filter(txn => txn.sourceContentId === content.id).length;
                const contentGMV = filteredTransactions
                  .filter(txn => txn.sourceContentId === content.id)
                  .reduce((sum, txn) => sum + txn.price, 0);
                const clicks = content.performance?.clicks || 0;
                const conversionRate = clicks > 0
                  ? ((contentDeals / clicks) * 100).toFixed(1)
                  : '0';

                return (
                  <div key={content.id} className="bg-bg-tertiary rounded-lg p-4 border border-gray-200">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500">{content.id} · {selectedHotelIds.length > 0 ? selectedHotelIds[0] : '标准房'}</div>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-6 gap-4 text-sm">
                      ...
                      <div className="text-center">
                        <div className="text-gray-500 mb-1">转化率</div>
                        <div className="font-mono">{conversionRate}%</div>
                      </div>
                      <div className="text-center">
                        <div className="text-gray-500 mb-1">成交GMV</div>
        ...
        {/* 漏斗可视化 */}
        <div className="col-span-4 bg-gray-100 rounded-xl border border-gray-200 p-6">
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-500">成交订单</span>
