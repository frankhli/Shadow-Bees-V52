    ...
    return (
      <Card className="p-4 bg-bg-secondary border-border-color">
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge className="bg-gray-500/20 text-text-secondary border-gray-500/30">
        ...
        {!pendingPriceApproval ? (
          <>
            ...
            <div className="mt-4 p-3 bg-[#00F0FF]/10 border border-[#00F0FF]/30 rounded-lg text-xs text-[#00F0FF]">
              ...
              <div className="text-text-secondary">作为员工，您无法直接修改价格。您的申请将发送给经理或老板审批。</div>
            </div>
          </>
        ) : (
          // 申请已提交状态 - 显示确认信息
  ...
  return (
    <Card className="p-4 bg-bg-secondary border-border-color">
      ...
      <div className="mt-4 p-3 bg-[#00E396]/10 border border-[#00E396]/30 rounded-lg text-xs text-[#00E396]">
        ...
        {user.role === 'owner' ? (
          <div className="text-text-secondary">您是老板，可以修改底价，审批员工申请，所有操作将被审计记录。</div>
        ) : (
          <div className="text-text-secondary">您是经理，可以调价（但不能低于底价），可以审批员工申请。</div>
        )}
      </div>
    </Card>
  );
