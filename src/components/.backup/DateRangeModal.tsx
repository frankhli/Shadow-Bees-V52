      ...
      >
        <div className="bg-bg-secondary rounded-xl border border-border-color p-6 shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-2 mb-6">
            {quickOptions.map((option) => (
              <button
                ...
                className="px-4 py-2 rounded-lg bg-bg-primary border border-border-color text-sm text-gray-200 hover:border-[#00F0FF]/50 hover:text-[#00F0FF] transition-all"
              >
                {option.label}
              </button>
            ))}
          ...
          <div className="space-y-4 mb-6">
            <div>
              <label className="block text-sm text-gray-300 mb-2">开始日期</label>
              <input
                type="date"
                value={startDate}
                max={new Date().toISOString().split('T')[0]}
                onChange={(e) => setStartDate(e.target.value)}
                className="w-full px-4 py-2 bg-bg-primary border border-border-color rounded-lg text-gray-200 focus:outline-none focus:border-[#00F0FF]/50 cursor-pointer"
                style={{ colorScheme: 'dark' }}
              />
            </div>
            <div>
              <label className="block text-sm text-gray-300 mb-2">结束日期</label>
              <input
                type="date"
                value={endDate}
                max={new Date().toISOString().split('T')[0]}
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 px-4 py-2 rounded-lg border border-border-color text-gray-300 hover:bg-border-color transition-colors"
            >
              取消
            </button>
            <button
