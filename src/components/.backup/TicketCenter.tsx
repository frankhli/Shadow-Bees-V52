  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        <div className="p-4 bg-[#151B2B] rounded-xl border border-gray-800 cursor-pointer hover:border-gray-600" onClick={() => setFilter('all')}>
          <p className="text-2xl font-bold">{stats.all}</p>
          <p className="text-xs text-text-secondary">全部工单</p>
        </div>
        <div className="p-4 bg-[#151B2B] rounded-xl border border-gray-800 cursor-pointer hover:border-amber-500/50" onClick={() => setFilter('open')}>
          <p className="text-2xl font-bold text-amber-400">{stats.open}</p>
          <p className="text-xs text-text-secondary">待处理</p>
        </div>
        <div className="p-4 bg-[#151B2B] rounded-xl border border-gray-800 cursor-pointer hover:border-cyan-500/50" onClick={() => setFilter('processing')}>
          <p className="text-2xl font-bold text-cyan-400">{stats.processing}</p>
          <p className="text-xs text-text-secondary">处理中</p>
        </div>
        <div className="p-4 bg-[#151B2B] rounded-xl border border-gray-800 cursor-pointer hover:border-purple-500/50" onClick={() => setFilter('pending_rating')}>
          <p className="text-2xl font-bold text-purple-400">{stats.pendingRating}</p>
          <p className="text-xs text-text-secondary">待评价</p>
        </div>
        <div className="p-4 bg-[#151B2B] rounded-xl border border-gray-800 cursor-pointer hover:border-green-500/50" onClick={() => setFilter('rated')}>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <select
          ...
          className="px-3 py-1.5 bg-[#151B2B] border border-gray-700 rounded-lg text-sm"
        >
        ...
        {filteredTickets.length === 0 ? (
          ...
          filteredTickets.map((ticket) => (
            <motion.div
              ...
              className="p-4 bg-[#151B2B] rounded-xl border border-gray-800 cursor-pointer hover:border-neon-cyan/50 transition-all"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative w-full max-w-xl bg-[#151B2B] rounded-xl border border-gray-800 overflow-hidden max-h-[90vh] flex flex-col"
      >
        {/* 头部 */}
          ...
          {step === 1 ? (
            <div className="space-y-4">
                    ...
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${
                        selectedIssue === issue.value && !isCustom
                          ...
                          : 'border-gray-700 text-text-secondary hover:border-gray-600 hover:bg-gray-800/50'
                      }`}
                    >
                ...
                {isCustom && (
                  ...
                  >
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-4 py-3 bg-bg-primary border border-gray-700 rounded-xl text-sm focus:border-neon-cyan focus:outline-none resize-none"
                    />
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              {(selectedIssue || (isCustom && customDescription)) && !isCustom && (
                <div className="p-4 bg-bg-primary rounded-xl border border-gray-800">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-bg-primary border border-gray-700 rounded-xl text-sm focus:border-neon-cyan focus:outline-none"
                  />
                </div>
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-300 font-mono">{ticket.id}</span>
          </div>
          <button onClick={onClose} className="p-1 hover:bg-gray-800 rounded">
            <X size={20} className="text-text-secondary" />
          </button>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          ...
          <div>
            ...
            {(ticket.contactName || ticket.contactPhone || ticket.contactEmail) && (
              <div className="mt-3 p-3 bg-bg-primary rounded-lg border border-gray-700">
            ...
            <div className="flex items-center gap-4 mt-3 text-xs text-text-secondary">
              <span className="text-gray-300">{ticketTypeLabels[ticket.type]}</span>
          ...
          {ticket.status !== 'resolved' && ticket.status !== 'closed' && (
            <div className="p-4 bg-bg-primary rounded-lg border border-gray-800">
              {/* 负责人 */}
              {ticket.assignedToName && (
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm text-gray-300">处理人：{ticket.assignedToName}</span>
          ...
          {ticket.status === 'resolved' && ticket.rating && (
            <div className="p-4 bg-green-500/5 border border-green-500/20 rounded-lg space-y-3">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-300">整体满意度</span>
          ...
          {(showRating || (ticket.status === 'resolved' && !ticket.rating)) && (
            <div className="p-4 bg-bg-primary rounded-lg space-y-4">
              ...
              {/* 整体满意度 */}
              <div className="space-y-2">
                <label className="text-sm text-gray-300">整体满意度</label>
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
              ...
              {/* 响应速度 */}
              <div className="space-y-2">
                <label className="text-sm text-gray-300">响应速度</label>
              ...
              {/* 解决效果 */}
              <div className="space-y-2">
                <label className="text-sm text-gray-300">解决效果</label>
                <div className="flex gap-2">
                  {[
              ...
              {/* 评价标签 */}
              <div className="space-y-2">
                <label className="text-sm text-gray-300">评价标签（可多选）</label>
                <div className="flex flex-wrap gap-2">
              ...
              {/* 文字反馈 */}
              <div className="space-y-2">
                <label className="text-sm text-gray-300">其他建议（选填）</label>
                <textarea
                  value={feedback}
                  onChange={(e) => setFeedback(e.target.value)}
                  placeholder="您的建议对我们很重要..."
                  rows={3}
                  className="w-full px-3 py-2 bg-[#151B2B] border border-gray-700 rounded-lg text-sm text-gray-200 placeholder-gray-500 resize-none focus:border-neon-cyan focus:outline-none"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="px-4 py-2 text-sm text-gray-300 hover:text-text-primary transition-colors"
                >
                  取消
                </button>
                <button
            ...
            {ticket.status !== 'resolved' && (
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="输入回复..."
                  className="flex-1 px-4 py-2 bg-bg-primary border border-gray-700 rounded-lg text-sm focus:border-neon-cyan focus:outline-none"
